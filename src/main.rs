...
use vulkano::instance::{Instance, ApplicationInfo, Version, PhysicalDevice};
...
fn main() {
  ...
  let instance = {
      ...
      info!("vulkano_win required extensions: {:?}", extensions);
      ...
      info!("application info: {:?}", app_info);
  ...
  for physical_device in PhysicalDevice::enumerate(&instance) {
      info!("Available device: {}", physical_device.name());
  }
  ...
  let uniform_buffer = vulkano::buffer::cpu_pool::CpuBufferPool::<shader::fs::ty::Data>
                              ::new(device.clone(), vulkano::buffer::BufferUsage::all());

  // let vs = unsafe { vs::Shader::load_binary(device.clone(), vs::SPIRV_BINARY).expect("failed to create shader module") };
  // let fs = unsafe { fs::Shader::load_binary(device.clone(), fs::SPIRV_BINARY).expect("failed to create shader module") };
  ...
  let create_pipeline = |fs: shader::fs::Shader| {
    info!("recreating pipeline...");
  ...
  let mut pipeline = create_pipeline(fs);
  ...
  info!("swapchain format: {:?}", swapchain.format());
  ...
  loop {
      ...
      if !wrote_screenshot && false {
        if let Ok(screenshot_buffer_contents) = screenshot_buffer.read() {
            info!("writing screenshot buffer");
      ...
      match future {
          ...
          Err(error) => {
            error!("error rendering previous frame: {}", error);
            previous_frame_end = Box::new(vulkano::sync::now(device.clone())) as Box<_>;
          }
      }
